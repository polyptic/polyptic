 ...
 *
 * OPERATOR BRANDING (POL-194): the same swap, without editing the repo. `setup --brand-mark <file>
 * --brand-wordmark <name>` puts a customer's own mark and word into that lockup, and the image build
 * pulls both off the control plane before it runs setup in the chroot — so a box boots into the
 * customer's brand rather than ours. The lockup itself is generated by `logoSvg()` in
 * `@polyptic/protocol`, which is also what the GRUB theme and the console's preview render.
 */

import { SPLASH_COLORS, escapeXml, logoSvg } from "@polyptic/protocol";
...
// ── palette + lockup: ONE definition, in the shared contract ─────────────────────────────────────
// Both live in `@polyptic/protocol` (POL-194) because the agent is no longer their only renderer:
// the control plane rasterises the same lockup for `GET /boot/logo.png` and the console draws it as
// the Branding preview. Re-exported here so every existing importer of `SPLASH_COLORS` / `logoSvg`
// (the GRUB theme, `deploy/render-boot-logo.ts`, the e2e suites) is unchanged.
const COLORS = SPLASH_COLORS;
export { SPLASH_COLORS, logoSvg };
...
export function stampSvg(p: StampParams): string {
  const host = escapeXml(p.hostname);
  const version = escapeXml(p.version.startsWith("v") ? p.version : `v${p.version}`);
 ...
 *
 * `brand` (POL-194) is the operator's mark + wordmark, pulled off the control plane by the image
 * build and handed to `polyptic-agent setup` on the command line. Absent — the zero-touch
 * `curl|sh` path, a laptop provision, a control plane with no brand saved — the lockup is the
 * Polyptic one, exactly as before.
 */
export function splashAssets(stamp: StampParams, brand: BrandInputs = {}): SplashAsset[] {
  return [
    { base: "logo", svg: logoSvg(brand), width: 1600 },
