/**
 * `/etc/polyptych/agent.toml` — the agent's on-box configuration file (D26).
 *
 * This module is the single home for that file's shape: it is BOTH
 *   - rendered by `polyptych-agent setup` (writes the file from flags), and
 ...
 * `/etc/polyptych/agent.toml` behaves exactly as before (Phase 1/2 parity).
 */
import { readFileSync } from "node:fs";

export const DEFAULT_CONFIG_PATH = "/etc/polyptych/agent.toml";

/** Maps `agent.toml` keys onto the `POLYPTYCH_*` env vars the agent already reads. */
const KEY_TO_ENV: Readonly<Record<string, string>> = {
  server_url: "POLYPTYCH_SERVER_URL",
  bootstrap_token: "POLYPTYCH_BOOTSTRAP_TOKEN",
  backend: "POLYPTYCH_BACKEND",
  connector: "POLYPTYCH_CONNECTOR",
  machine_id: "POLYPTYCH_MACHINE_ID",
  state_dir: "POLYPTYCH_STATE_DIR",
};
...
/**
 * Seed `process.env` defaults from the agent config file (path from `POLYPTYCH_CONFIG`, else the
 * default). Only sets a `POLYPTYCH_*` var that is not already present/non-empty, so real env always
 * wins. Absent/unreadable file → no-op.
 */
export function applyConfigFileToEnv(env: NodeJS.ProcessEnv = process.env): void {
  const path = env.POLYPTYCH_CONFIG?.trim() || DEFAULT_CONFIG_PATH;
  let text: string;
  try {
...
/** Render `/etc/polyptych/agent.toml` (or its `.example`) from setup flags. */
export function renderAgentToml(v: AgentTomlValues, opts: { example: boolean }): string {
  ...
  const header = opts.example
    ? "# EXAMPLE — copy to /etc/polyptych/agent.toml and fill in, then restart the agent."
    : "# Written by `polyptych-agent setup`. Edit, then restart the agent.";
  const lines = [
    "# /etc/polyptych/agent.toml — Polyptych agent configuration (D26).",
    header,
    "#   systemctl --user restart polyptych-agent      # from inside the kiosk session",
    "#",
    "# Real environment variables (POLYPTYCH_*) and the systemd unit's Environment= override these.",
    "",
