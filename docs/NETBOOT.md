...
**Re-installing a box that already has an install** puts two POLYPTIC-BT volumes carrying a marker in front of the same box — the stick it booted, and the ESP of the install being replaced — so the by-label symlink is ambiguous and udev may publish either. The medium is therefore identified by the **running boot chain**: the live device dmsquash-live mounted (`/run/initramfs/live`), the cmdline's live root spec (`root=live:LABEL=…`), and the `disk-esp-` marker stamp, which is accepted only on a boot that came from that disk. A box booted from the stick reads the stick; an installed box booted from its own disk reads its own ESP; and the installer asks the same script (`find-boot-medium.sh --booted-disk`) which disk carried the boot chain, because *never wipe the disk this box booted from* is a question about the boot chain, not about which volume answered a label. Re-installing therefore means booting the box from the USB medium first — install-to-disk refuses to wipe the disk it is running from, and says so.
