...
- `resolution-before-firmware-mode.png` / `resolution-after-native.png` — the POL-53
  ([D56](../../DECISIONS.md)) comparison, both as a 1440p panel sees them. **Before**: the initramfs
  carried no KMS driver, so plymouth composed the splash on the firmware's 1280×800 framebuffer and
  the panel upscaled it. **After**: the `drm` dracut module is in the initramfs, the real driver
  mode-sets the connector's preferred mode, and the splash is composed at 2560×1440.
