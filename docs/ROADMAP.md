...
> **CURRENT: feature-complete through Phase 8 (skipping 6/OIDC). Pending = the UTM/OrbStack VM passes + the prod `vite build`/Docker (esbuild is disabled in the build sandbox).** Phase 3 ✅ (3a–3f). 3c content library (ContentSource) · 3d scenes (snapshot/apply/scenes-left) · 3e Machines view + cold-start wizard (SolidJS admin retired) · 3f real local auth (D29/D31 — argon2id/Bun.password, signed httpOnly cookies, dual-key rate-limit, /api+/admin-WS gated, AUTH_ENABLED default-on) + Settings + enrollment-token. **Phase 5 ✅** live preview thumbnails + /healthz + Prometheus /metrics + Helm chart (D32). **Phase 4 ✅** on-device stack (real sway/x11, .deb/.rpm). **Phase 7 ✅** media upload→disk volume + ungated Range serve (D33). **Phase 8 ✅** single-image SPAs + GitHub Actions CI/release/Pages + distribution docs (D34). Verified: full typecheck green, **130/130 tests across 14 files** (e2e + agent/server unit). **POL-9 fixed (D36):** no phantom `HDMI-1` before a compositor — a real backend that can't discover any outputs (and has no explicit override) advertises **zero** outputs, and the server prunes UNUSED connector-stragglers on re-advertise. **POL-27 done (D38):** a player with no surfaces now shows a calm "No Content Assigned" idle splash (dark, branded, screen id + build version) instead of a black screen. Next real work: run the VM passes (DEPLOY.md), then OIDC (6) when wanted.
