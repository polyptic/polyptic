...
> **CURRENT: Phase 3c next.** **Phase 3a ✅** Vue console + Wall murals canvas + server murals/placement + the **Polyptic logo** (:5175). **Phase 3b ✅** combined surfaces (video walls): `VideoWall` + `Surface.span`, server combine/split + the union-bbox span computation, the **player rewritten SolidJS→Vue** (preserves the instant in-place swap; renders span slices), console combine/split + the combined-surface inspector; unplacing a member / deleting a mural **dissolves** the wall (D30). **Phase 4 ✅ committed (pending the UTM-VM visual cold-boot test)** — real sway/x11 backends + `apt install` packaging. Verified: full typecheck green, e2e **41/41**. Sub-phases left: **3c** content library → 3d scenes (incl. scenes-left layout) → 3e machines view + cold-start → 3f sign-in (local auth) + settings. Then 5 preview/health/Helm · 6 OIDC · 7 media.
