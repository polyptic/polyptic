...
> **CURRENT: Phase 5 next. Phase 3 ✅ COMPLETE (3a–3f).** 3a console+murals+logo · 3b combined surfaces (VideoWall + Surface.span; player→Vue; dissolve D30) · 3c content library (ContentSource) · 3d scenes (snapshot/apply/scenes-left) · 3e Machines view + cold-start wizard (SolidJS admin retired) · 3f real local auth (D29 — argon2id/Bun.password, signed httpOnly cookies, dual-key rate-limit, /api+/admin-WS gated, AUTH_ENABLED default-on) + Settings + enrollment-token. **Phase 4 ✅ committed (pending the UTM-VM visual cold-boot test).** Verified: full typecheck green, e2e **72/72**. Remaining (overnight autonomous run): **5** preview/health/Helm → **7** media (linkable+uploadable, disk volume) → **8** packaging/distribution (CI/release; no push). **Skipping 6 (OIDC).** Then the /explain doc.
