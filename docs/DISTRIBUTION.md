...
How Polyptic ships, and how you put it on real infrastructure. This is the **packaging** guide: the artifacts, where they come from, and the exact commands to run them. For turning a bare Linux box into a display see `docs/DEPLOY.md`. For the dev loop see `docs/DEV.md`.

> **Nothing here auto-publishes.** The release workflow only runs when *you* push a `vX.Y.Z` tag, and the image and registry steps need secrets that don't exist until you create them. You choose when (and whether) anything leaves your machine.
...
| **Netboot** (diskless machine, *no OS install*) | a **live image** + a universal **boot medium** `polyptic-boot.img` (Ubuntu's **signed shim + GRUB**, so **Secure Boot stays ON**), served by the depot | UEFI HTTP Boot / a USB dongle → the machine streams into RAM | nothing persists on the machine, see [NETBOOT.md](NETBOOT.md) |
| **Workspace packages** (`@polyptic/*`) | npm tarballs | *optional*, internal to the product | a private registry, only if you want one |
...
You bring your own Postgres (any reachable Postgres 16, with `DATABASE_URL` pointing at it). Put a TLS-terminating reverse proxy in front, and the https `PUBLIC_BASE_URL` flips the session cookie `Secure` automatically. Without a proxy, set `TLS_CERT_FILE`/`TLS_KEY_FILE` (mounted PEMs) and the server terminates TLS itself.
...
The chart in `deploy/helm/polyptic` deploys the **server and its database**. Agents live on the boxes, not in the cluster. One command and nothing to apply first, because `postgresql.enabled` defaults to true and the chart brings up its own Postgres (a first-party StatefulSet, not a subchart), generating the password and wiring `DATABASE_URL` for you.
...
Point it at a managed Postgres instead with `--set postgresql.enabled=false --set externalDatabase.url=postgres://…`. Setting **both** is refused at template time, as is `config.store=postgres` with **neither**, because that combination would install happily and then crash-loop the server on `getaddrinfo ENOTFOUND`.

If you leave `secrets.cookieSecret` empty the chart generates one on first install and preserves it across upgrades (the bundled database's password works the same way). Liveness and readiness hit the **ungated** `/healthz`, and Prometheus scrape annotations target the ungated `/metrics`. See `deploy/helm/polyptic/README.md` for the full values reference, including how to adopt an existing `polyptic-db` PVC.
...
| `PUBLIC_BASE_URL` | The ONE public origin (`scheme://host[:port]`) | **Set it in prod.** `https://` turns Secure cookies on automatically. A declared `http://` turns them off (browsers drop Secure cookies over http, so login would never persist) with a loud boot banner. |
...
| `TLS_CERT_FILE` / `TLS_KEY_FILE` | Native TLS: serve the whole listener over https | PEM paths, **both together** or the server refuses to boot. Prefer a terminating ingress/proxy, because native TLS also makes the netboot depot https, which GRUB cannot fetch. |
...
| `MEDIA_PUBLIC_BASE` | Absolute base URL prepended to `/media/<id>` | Must be reachable by players and the public wall, so use your public HTTPS origin. |
...
The per-machine agent is **not** in the server image. There is no standalone `.deb`/`.rpm` to
`apt install` and no `curl … | sh` installer. A machine becomes a Polyptic display exactly one
way, by **network-booting the live image the control plane serves**:

1. Console → **Settings → Onboard Screens** → **Download bootloader**.
2. Flash `polyptic-boot.img` to a USB stick (2 GB or larger).
...
agent → browser per output, plus the boot splash), wired at **build** time by `polyptic-agent setup`
...
typed on the machine and no config file is edited. The machine dials home and shows **PENDING** in the
console until an operator **Approves** it.

Because a diskless machine re-pulls its whole OS at every boot, "update the agent" and "update the
image" are the same operation, and it is automatic (see *Update schedule* in Settings).

The full device story (backends, multi-output placement, crash hardening, troubleshooting, the VM
...
| `GET /dist/agent/:arch` | the prebuilt agent **binary** (`arch` ∈ `arm64`\|`amd64`), streamed; `404` if not bundled. **No boot path fetches this** because the image bakes the binary in. Kept for agent over-the-air updates | `AGENT_DIST_DIR/polyptic-agent-<arch>` (default `./deploy/dist`) |

All paths are traversal-safe and 404 cleanly when an artifact is absent.

### How the artifacts get baked in (the server image)

`deploy/server.Dockerfile` is the normal way to fill the depot, with no extra steps for the common case:

- **Agent binaries, both arches.** The build stage cross-compiles the agent with `bun build --compile --target=bun-linux-x64` **and** `--target=bun-linux-arm64` into `/app/deploy/dist/polyptic-agent-{amd64,arm64}`. One image build serves every machine with no per-arch build host, because Bun bakes the runtime into each binary. `build-live-image.sh` installs the matching one into the live rootfs.
- The runtime stage copies `deploy/dist` and the image-rebuild scripts, and sets `AGENT_DIST_DIR=/app/deploy/dist`.
- **The live image + boot medium** are built by `deploy/build-live-image.sh` and `deploy/build-boot-medium.sh`, on a schedule or from the console's ⋯ menu, into `IMAGE_DIST_DIR` / `BOOT_DIST_DIR`.
...
Releases are driven entirely by **git tags**. Pushing a tag of the form `vX.Y.Z` triggers the release workflow. Nothing builds or publishes on an ordinary push to `main`.
...
1. **Builds the server image** and pushes it to GHCR as `ghcr.io/<owner>/polyptic-server:0.1.0` (and `:latest`). The image's build stage runs the `vite build` of the console + player and bakes the agent binaries (both arches) into the depot, with the version set via `--build-arg POLYPTIC_VERSION`.
...
3. **Creates the GitHub Release** for the tag and **attaches the agent binaries** so you can seed a depot's `AGENT_DIST_DIR` from the Release (the running server image already serves them, and there is no `.deb`/`.rpm` to `apt install`).
...
- No third-party registries and no cloud credentials. The chart's `appVersion` / image `tag` should match the tag you push (e.g. bump `Chart.yaml` `version`/`appVersion` to `0.1.0` in the commit you tag); the release workflow strips the leading `v` from the tag for the agent binary version.
...
## (d) Self-hosted npm (optional)

**You do not need this to deploy Polyptic.** The `@polyptic/*` workspace packages (`@polyptic/protocol`, `@polyptic/server`, `@polyptic/console`, `@polyptic/player`, `@polyptic/agent`) are **internal to the product**. They are wired together inside the monorepo and shipped *as* the Docker image and the depot-served agent binary. You deploy the image and the binary, and you never `npm install @polyptic/server` to run Polyptic. **Publishing to any npm registry is entirely optional**, and **nothing is published until you deliberately choose to.**

A registry is only useful when another codebase consumes these packages, for example a separate internal tool that imports `@polyptic/protocol`'s zod contracts, or a package split out of the monorepo later. If that day comes, here are the two sane paths.

### Recommended: GitHub Packages (lowest friction)

GitHub Packages gives you an org-scoped private npm registry with **zero new infrastructure**: the same GitHub account, the same `GITHUB_TOKEN` and the same access model as your code and your GHCR images. Recommended unless you specifically need to be off GitHub.
...
If you want the registry **entirely under your control** (air-gapped, on your own host, no GitHub dependency), run **[Verdaccio](https://verdaccio.org/)**, a lightweight self-hosted npm registry and caching proxy. `docker run -d -p 4873:4873 -v verdaccio-storage:/verdaccio/storage verdaccio/verdaccio` stands it up. Create a user with `npm adduser --registry http://your-host:4873`, point the scope at it with the `.npmrc` line `@polyptic:registry=http://your-host:4873`, then `npm publish --registry http://your-host:4873`. Verdaccio transparently proxies the public npm registry for everything *not* under `@polyptic`, so a single registry URL serves both your private scope and public deps. Put TLS + auth in front of it for anything beyond a trusted LAN. Verdaccio is the right choice when policy forbids storing packages on GitHub. Otherwise GitHub Packages is less to operate.
...
A self-hosted fleet sits on a network that has a security function, and that function will want its own software on every box: endpoint protection, a vulnerability scanner, an inventory agent, internal CA trust, log forwarding. The **site layer** is the supported seam for all of it, so nobody has to fork the build.
...
An absent or empty `deploy/site` is an exact no-op; `SITE_DIR` overrides the location.

**The rule that shapes it: packages install at build time, registration happens on the box at boot.** Registration writes a bearer credential to disk, and the image is served **without authentication** (see (b2) above), because a box has no session before it has booted. Register during the build and you publish your own credential. So secrets ride the boot medium as `polyptic/site.conf`, next to the Wi-Fi credentials, and reach your hooks in RAM.

**`MIRROR` is the related setting** for an organisation that would rather build against its own Ubuntu mirror than the public archive:
...
**→ Full guide: [`docs/SITE-LAYER.md`](SITE-LAYER.md)** — the bundle reference, the secrets format, the agent-identity trap that makes a whole fleet report as one host, a worked example, verification, and an honest list of what this model does and does not satisfy for a security function.
...
- `docs/DEV.md`: local dev, and bringing up the control plane for testing.
